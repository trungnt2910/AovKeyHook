...
namespace AovKeyHook
{
    class Hooker
    {
        ...
        static extern bool PostMessage(IntPtr hWnd, uint Msg, IntPtr wParam, UIntPtr lParam);
        ...
        static extern int GetProcessDpiAwareness(IntPtr hprocess, out uint value);
        ...
        static readonly WinEventDelegate mainDelegate = HandleEvents;
        static IntPtr _hookHandle;
        static IKeyboardMouseEvents _globalHook;
        static bool _isAovEnabled = false;
        static Rect _aovRect;
        static IntPtr _aovHwnd;
        static HashSet<Keys> _keysDown = new HashSet<Keys>();

        const string windowName = "Liên Quân Mobile";
        const string genericWindowName = "Windows Subsystem for Android™";

        internal static void HookMain(string[] args)
        {
            SetProcessDpiAwareness(2);

            GetProcessDpiAwareness(Process.GetCurrentProcess().Handle, out uint value);

            var hwnd = FindWindow(null, windowName);
            if (hwnd == IntPtr.Zero)
            {
                hwnd = FindWindow(null, genericWindowName);
            }

            GetWindowThreadProcessId(hwnd, out uint pid);
            _hookHandle = SetWinEventHook(WinEvents.EVENT_MIN, WinEvents.EVENT_MAX, IntPtr.Zero, mainDelegate, 0, 0, WinEventFlags.WINEVENT_OUTOFCONTEXT);
            _globalHook = Hook.GlobalEvents();
            _globalHook.KeyDown += HandleKeyDown;
            _globalHook.KeyUp += HandleKeyUp;
            _globalHook.MouseClick += HandleMouseClick;
            _globalHook.MouseDownExt += HandleMouseDown;
            _globalHook.MouseUpExt += HandleMouseUp;
            _globalHook.MouseMoveExt += HandleMouseMoveExt;

            if (hwnd != IntPtr.Zero)
            {
                SetForegroundWindow(hwnd);
                Fingers.SetWindow(hwnd);
            }
        }

        public static async Task ClickAsync(Point point)
        {
            // Can't use the global guy.
            var hwnd = FindWindow(null, windowName);
            if (hwnd == IntPtr.Zero)
            {
                hwnd = FindWindow(null, genericWindowName);
            ...
            if (hwnd != IntPtr.Zero)
            {
                //Fingers.SetRightStatus(point, true);
                //await Task.Delay(200);
                //await Fingers.WaitForNextFrame();
                //Fingers.SetRightStatus(point, false);

                GetWindowRect(_aovHwnd, out var rect);
                var size = new Size(rect.Right - rect.Left, rect.Bottom - rect.Top);
                var xRatio = size.Width / GameMap.ScreenSize.Width;
                var yRatio = size.Height / GameMap.ScreenSize.Height;
                ...
                TouchSimulator.SimulateTap(_aovHwnd, point, 1);

                if (_captured)
                {
                    Console.WriteLine("Posting message to game.");
                    PostMessage(hwnd, WM_LBUTTONDOWN, (IntPtr)MK_LBUTTON, ((UIntPtr)(((int)GameMap.JoystickCenter.X) | (((int)GameMap.JoystickCenter.Y) << 16))));
                    Console.WriteLine($"{oldPoint.x},{oldPoint.y}");
                    SetCursorPos(oldPoint.x, oldPoint.y);
                }

                //PostMessage(hwnd, WM_LBUTTONDOWN, (IntPtr)MK_LBUTTON, ((UIntPtr)(((int)point.X) | (((int)point.Y) << 16))));
                //await Task.Delay(100);
                //PostMessage(hwnd, WM_LBUTTONUP, (IntPtr)0, ((UIntPtr)(((int)point.X) | (((int)point.Y) << 16))));
        ...
        private static Rect GetWindowRect(IntPtr handle)
        {
            ...
            return new Rect(rect.Left / dpi, rect.Top / dpi, (rect.Right - rect.Left) / dpi, (rect.Bottom - rect.Top) / dpi);
        }

        private static void HandleMouseClick(object sender, MouseEventArgs e)
        {
            if (_isAovEnabled)
            {
                var factor = GetScaleFactor(_aovHwnd);
            }
        }
        ...
        private static void HandleMouseUp(object sender, MouseEventExtArgs e)
        {
            /*if (_isAovEnabled && !_captured)
            {
                var point = new Point(e.X - _aovRect.Left, e.Y - _aovRect.Top);
                //TouchSimulator.SimulateFingerUp(_aovHwnd, point, 0);
                if (e.Timestamp - _timestamp < 500)
                {
                    Console.WriteLine($"Mouse double clicked. Capturing.");
                    _captured = true;
                    e.Handled = true;
                    _timestamp = 0;
                    //Fingers.SetLeftStatus(point, true);
                    GetWindowRect(_aovHwnd, out var rect);
            ...
            else */ if (_isAovEnabled)
            {
                var point = new Point(e.X - _aovRect.Left, e.Y - _aovRect.Top);
                e.Handled = false;
                Console.WriteLine("Releasing mouse.");
                _captured = false;
                // Avoiding re-capture.
                _timestamp = 0;
                ClipCursor(IntPtr.Zero);
                //Fingers.SetLeftStatus(null, false);
            }
            else
            {
                e.Handled = false;
        ...
        private static void HandleMouseDown(object sender, MouseEventExtArgs e)
        {
            //if (_isAovEnabled)
            //{
            //    _captured = true;
            //    e.Handled = true;
                var point = new Point(e.X - _aovRect.Left, e.Y - _aovRect.Top);
                Console.WriteLine($"Mouse down: {point}");
            //    //TouchSimulator.SimulateFingerDown(_aovHwnd, point, 0);
            //}
            //else
            //{
            //    e.Handled = false;
        ...
        private static void HandleMouseMoveExt(object sender, MouseEventExtArgs e)
        {
            if (_isAovEnabled && _captured)
            {
                //e.Handled = true;
                var point = new Point(e.X - _aovRect.Left, e.Y - _aovRect.Top);
                Fingers.SetLeftStatus(point, null);
                //TouchSimulator.SimulateFingerMove(_aovHwnd, point, 0);
            }
        }

        private static async void HandleKeyUp(object sender, KeyEventArgs args)
        {
            if (_isAovEnabled)
            {
                var point = GameMap.HandleKeyUp(args.KeyCode);
                if (point != null)
                {
                    await ClickAsync(point.Value);
                }
                _keysDown.Remove(args.KeyCode);
            }
        }

        private static async void HandleKeyDown(object sender, KeyEventArgs args)
        {
            if (_isAovEnabled)
            {
                var point = GameMap.HandleKeyDown(args.KeyCode);
                if (point != null && !_keysDown.Contains(args.KeyCode))
                {
                    await ClickAsync(point.Value);
                }
        ...
        internal static void UnhookMain()
        {
            UnhookWinEvent(_hookHandle);
            _globalHook.Dispose();
        }

        static void HandleEvents(IntPtr hook, uint eventType, IntPtr hwnd, int idObject, int idChild, uint dwEventThread, uint dwmsEventTime)
        {
            switch ((WinEvents)eventType)
            {
                case WinEvents.EVENT_SYSTEM_FOREGROUND:
                    var aovHwnd = FindWindow(null, windowName);
                    if (aovHwnd == IntPtr.Zero)
                    {
                        aovHwnd = FindWindow(null, genericWindowName);
                    }
                    _isAovEnabled = aovHwnd == hwnd;
                    if (_isAovEnabled)
                    {
                        _aovRect = GetWindowRect(aovHwnd);
                        _aovHwnd = hwnd;
                        Fingers.SetWindow(_aovHwnd);
                    }
                    else
                    {
                        _captured = false;
                ...
                case WinEvents.EVENT_OBJECT_LOCATIONCHANGE:
                    if (_isAovEnabled && hwnd == _aovHwnd)
                    {
                        Console.WriteLine("Window resized.");
                        _aovRect = GetWindowRect(hwnd);
                        Console.WriteLine($"Window rect: {_aovRect.Left}, {_aovRect.Right}, {_aovRect.Top}, {_aovRect.Bottom}, size: {_aovRect.Width}x{_aovRect.Height}");
                    }
                    break;
                default:
                    break;
